...
int tshark_parseFile_extractSS(const char *fileName) {
	...
	char arrivalTime[500], protocols[500];
	...
	fp = fopen("ICMP_SS_Values.csv","w");	/*register SS values here*/
	...
	while (!feof(input)) {
         ...
         */
       	...
       	while(1) {
           	...
           	fgets(protocols, sizeof(protocols), input);
           	if (!strstr( protocols, "Protocols in frame"))
                continue;
           	else
              	break;
       	}
		if(! strstr(protocols, "icmp"))  /* we r just using packets from STA to DS via an AP */
			continue;
