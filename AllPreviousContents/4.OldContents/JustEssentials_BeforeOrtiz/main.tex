...
We propose a solution in which the received signal
strength is modeled as a Gaussian Mixture Model (GMM). Expectation
maximization (EM) is used to find the parameters of our GMM. This approach 
is able to provide
a location fix for a transmitting device based on the maximum
likelihood estimate. This way, not only the costly
training phase is avoided but the location estimates are much more
